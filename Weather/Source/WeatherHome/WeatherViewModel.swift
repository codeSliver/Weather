...
extension WeatherViewModel {
    ...
    /// Fetches offline weather from bundled json file
    func fetchOfflineWeather() {
        weatherService.fetchWeatherForCity(nil, completion: { [weak self] (forcastResult, error) in
            self?.handlerForcastResponse(forcastResult, error: error)
        })
    }
    
    /// Fetch weather  data for default city on app launch
    func fetchWeatherForDefaultCity() {
        weatherService.fetchWeatherForCity(WeatherAPIKey.defaultCityID) { [weak self]  (forcastResult, error) in
            self?.handlerForcastResponse(forcastResult, error: error)
        }
    }
    
    /// Fetch weather  for specific city selected  by user
